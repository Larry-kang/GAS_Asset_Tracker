...
若要執行優化項目，建議順序：
1. **P1.5**: LogService 覆蓋度（優先執行，提升可追蹤性）
2. **P2-3**: API 憑證管理（最有實際價值）
3. **P2-5**: Webhook 文件化（提升專案完整度）
4. **P2-4**: 版本號常量（易於維護）
5. **P2-6**: IMPORTJSON 標記（改善工具完整性）
6. **P2-2**: 測試檔案文件（開發體驗）
7. **P2-1**: Legacy 命名檢視（最後處理）

...
| `API_Summary_Export` | Optional。供 `get_inventory` webhook 讀取的 row-based summary contract，至少要有 `key` / `value` 欄。 |
| `API_Position_Export` | Optional。供 `get_inventory` webhook 讀取的 header-driven position contract，至少要有 `ticker` 欄。 |

工作表結構由 `Lib_WorkbookContracts.js` 與 `Util_HealthCheck.js` 檢查。任何結構調整後，都應先跑 `runSystemHealthCheck()`。

### `get_inventory` 的彈性 API 契約

若只依賴既有 `Balance Sheet`，每次新增 API 欄位都需要改 Apps Script 後再 redeploy。為了把欄位擴充改成 sheet-level work，`get_inventory` 現在支援兩張 optional export sheet：

- `API_Summary_Export`
  - 第 1 列為 header
  - 必填欄位：`key`, `value`
  - 其他欄位例如 `source`, `asOf`, `note` 會原樣保留在 `inventoryExport.summaryRows`
- `API_Position_Export`
  - 第 1 列為 header
  - 必填欄位：`ticker`
  - 其他欄位例如 `account`, `quantity`, `valueTwd`, `price`, `priceCurrency` 會原樣保留在 `inventoryExport.positions`

Webhook 回傳仍保留舊版 `buildContext()` payload；如果上述 export sheets 存在且有資料，會額外附上：
